...
void usage(void)
{
	fprintf(stderr, "Usage: gps-sdr-sim [options]\n"
		...
		"  -c <location>    ECEF X,Y,Z in meters (starting location) e.g. 3967283.154,1022538.181,4872414.484\n"
		"  -l <location>    Lat,Lon,Hgt (startomg location) e.g. 35.681298,139.766247,10.0\n"
...
int main(int argc, char *argv[])
{
	...
	HANDLE rfThread = NULL;
	HANDLE servThread = NULL;
	DWORD dwThreadId;
	...
	while ((result = getopt(argc, argv, "e:u:g:c:l:s:b:T:t:d:ivwf:a:x:h")) != -1)
	{
		switch (result)
		{
		case 'e':
	...
	// set a default location default to here
	...
	*/
	...
	// populate hackrf thread arguments
	hta.argc = argc;
	hta.argv = argv;
	
	// create intrathread queue
	queue = queueCreate();
	if (NULL == queue) {
		fprintf(stderr, "ERROR: Unable to allocate queue.\n");
		exit(1);
	}
	hta.queue = queue;
	...
	if (webflag) {
		// populate socket thread args struct
		sta.mutex = malloc(sizeof(pthread_mutex_t));
		sta.llhr[0] = llh[0] * R2D;
		sta.llhr[1] = llh[1] * R2D;
		sta.llhr[2] = llh[2];

		if (NULL == sta.mutex) {
			fprintf(stderr, "ERROR: Unable to allocate mutex.\n");
			exit(0);
		}
		sta.update = 0;
		pthread_mutex_init(sta.mutex, NULL);

		// start hackrf thread
		servThread = CreateThread(
			NULL,
			0,
			startServer,
			&sta,
	...
	if (NULL == rfThread) {
		fprintf(stderr, "ERROR: Unable to start hackrf thread.\n");
		exit(1);
	}

	for (iumd = 1; iumd < numd && running; iumd++)
	{
		for (i = 0; i < MAX_CHAN; i++)
		{
			if (chan[i].prn > 0)
...
#endif
					if (iTable >= sizeof(cosTable512) || iTable >= sizeof(sinTable512)) {
						continue;
					}
		...
		if (data_format == SC01)
		{
			...
			char *buf = malloc(iq_buff_size / 4);
			memcpy(buf, iq8_buff, iq_buff_size / 4);
			message *msg = malloc(sizeof(message));
			msg->buffer = buf;
			msg->length = iq_buff_size / 4;
			queuePush(queue, msg);
		...
		else if (data_format == SC08)
		{
			...
			char *buf = malloc(2 * iq_buff_size);
			memcpy(buf, iq8_buff, 2 * iq_buff_size);
			message *msg = malloc(sizeof(message));
			msg->buffer = buf;
			msg->length = 2 * iq_buff_size;
			queuePush(queue, msg);
		...
		{
			char *buf = malloc(2 * iq_buff_size * 2);
			memcpy(buf, iq_buff, 2 * iq_buff_size * 2);
			message *msg = malloc(sizeof(message));
			msg->buffer = buf;
			msg->length = 2 * iq_buff_size * 2;
			queuePush(queue, msg);
		...
		if (subGpsTime(grx, g0) - (float)(timem() - timestart) / 1000 > 0.1) {
			Sleep(100);
			//usleep(100000);
		}
		
		if (webflag == 1 && sta.update == 1) {
			pthread_mutex_lock(sta.mutex);
			memcpy(llh, sta.llhr, 3 * sizeof(double));
			sta.update = 0;
		...
		// Update time counter
		// fprintf(stderr, "Time into run = %4.1f %zd\n", subGpsTime(grx, g0), queue->numElements);
		fflush(stdout);
	}

	WaitForSingleObject(servThread, INFINITE);
	WaitForSingleObject(rfThread, INFINITE);
